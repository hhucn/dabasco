...
class DoJ(object):
    ...
    def doj_conditional(self,graph,pos,conditionalPos,_doj,_coh):
        if _doj == DoJ.DOJ_RECALL:
            return self.recall_conditional(graph,pos,conditionalPos,_coh)
        elif _doj == DoJ.DOJ_PRECISION:
            return self.precision_conditional(graph,pos,conditionalPos,_coh)
        #elif _doj == DoJ.DOJ_F1:
    ...
    def recall(self,graph,pos,_coh):
        emptyPos = Position(pos.n)
        n1 = graph.getNumberOfCoherentCompletions(pos,_coh)
        n2 = graph.getNumberOfCoherentCompletions(emptyPos,_coh)
        return n1/float(n2)
        
    def recall_conditional(self,graph,pos,conditionalPos,_coh):
        posUnion = conditionalPos.unionWith(pos)
        if posUnion == None:
            return 0.0
        n1 = graph.getNumberOfCoherentCompletions(posUnion,_coh)
        if n1 == 0:
            return 0.0
        n2 = graph.getNumberOfCoherentCompletions(conditionalPos,_coh)
        return n1/float(n2)
    ...
    def precision_conditional(self,graph,pos,conditionalPos,_coh):
        posUnion = conditionalPos.unionWith(pos)
        if posUnion == None:
            return 0.0
        n1 = graph.getNumberOfCoherentCompletions(posUnion,_coh)
        if n1 == 0:
            return 0.0
        n2 = posUnion.getNumberOfCompletions()
        return n1/float(n2)
    ...
    def reason1(self,graph,p,q,_doj,_coh):
        ...
        condPos = Position(graph.n)
        condPos.setAcceptance(q,Position.ACCEPTED)
        doj1 = self.doj_conditional(graph,pos,condPos,_doj,_coh)
    ...
    def reason2(self,graph,p,q,_doj,_coh):
        ...
        pos = Position(graph.n)
        pos.setAcceptance(p,Position.ACCEPTED)
        condPos1 = Position(graph.n)
        condPos1.setAcceptance(q,Position.ACCEPTED)
        condPos2 = Position(graph.n)
        condPos2.setAcceptance(q,Position.REJECTED)
        doj1 = self.doj_conditional(graph,pos,condPos1,_doj,_coh)
        doj2 = self.doj_conditional(graph,pos,condPos2,_doj,_coh)
        return doj1 - doj2
